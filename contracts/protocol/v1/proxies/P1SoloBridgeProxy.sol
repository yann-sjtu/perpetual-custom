 ...
 */
contract P1SoloBridgeProxy is P1Proxy, ReentrancyGuard {
    ...
    string private constant EIP712_DOMAIN_NAME = 'P1SoloBridgeProxy';
    ...
    /* solium-disable-next-line indentation */
    bytes32 private constant EIP712_TRANSFER_STRUCT_SCHEMA_HASH =
        keccak256(
            abi.encodePacked(
                'Transfer(',
                'address account,',
    ...
    bytes32 private constant OPTIONS_MASK_TRANSFER_MODE =
        bytes32(uint256((1 << 8) - 1));
    bytes32 private constant OPTIONS_MASK_EXPIRATION =
        bytes32(uint256((1 << 120) - 1));
    ...
    constructor(address soloMargin, uint256 chainId) public {
     ...
     */
    function approveMaximumOnSolo(uint256 soloMarketId) external {
        address solo = _SOLO_MARGIN_;
        IERC20 tokenContract = IERC20(
            I_Solo(solo).getMarketTokenAddress(soloMarketId)
        );
    ...
    function bridgeTransfer(
        ...
        bool toPerpetual = (transferMode == TransferMode.SomeToPerpetual ||
            transferMode == TransferMode.AllToPerpetual);

        // Validations.
        _verifySoloMarket(solo, transfer, tokenAddress);
        ...
        if (!isValidOperator) {
            _verifySignature(transfer, transferHash, signature);
        ...
        if (toPerpetual) {
            // Withdraw from Solo.
            uint256 initialBalance = IERC20(tokenAddress).balanceOf(
                address(this)
            );
            _doSoloOperation(
                solo,
            ...
            uint256 finalBalance = IERC20(tokenAddress).balanceOf(
                address(this)
            );
            ...
            // transferMode == TransferMode.SomeToSolo
            ...
            // Deposit to Solo.
            _doSoloOperation(solo, transfer, false, false);
        }
     ...
     */
    function invalidateSignature(Transfer calldata transfer) external {
        ...
        if (msg.sender != transfer.account) {
            ...
            bool toPerpetual = (transferMode == TransferMode.SomeToPerpetual ||
                transferMode == TransferMode.AllToPerpetual);
            require(
                _isValidOperator(solo, perpetual, transfer, toPerpetual),
        ...
        emit LogSignatureInvalidated(transfer.account, transferHash);
    }
    ...
    function _doSoloOperation(
        ...
        I_Solo.AssetAmount memory amount = I_Solo.AssetAmount({
            ...
            ref: withdrawToZero
                ? I_Solo.AssetReference.Target
                : I_Solo.AssetReference.Delta,
        ...
        soloActions[0] = I_Solo.ActionArgs({
            actionType: isWithdrawal
                ? I_Solo.ActionType.Withdraw
                : I_Solo.ActionType.Deposit,
    ...
    function _isValidOperator(
        ...
        bool isSoloOperator = solo.getIsGlobalOperator(msg.sender) ||
        ...
        return
            isSoloOperator &&
            (toPerpetual ||
                perpetual.hasAccountPermissions(transfer.account, msg.sender));
    }
     ...
     */
    function _getTransferHash(Transfer memory transfer)
        private
        view
        returns (bytes32)
    {
        ...
        bytes32 structHash = keccak256(
            abi.encode(EIP712_TRANSFER_STRUCT_SCHEMA_HASH, transfer)
        );
    ...
    function _getTransferMode(Transfer memory transfer)
        private
        pure
        returns (TransferMode)
    {
        return
            TransferMode(
                uint256(transfer.options & OPTIONS_MASK_TRANSFER_MODE)
            );
    }

    function _getExpiration(Transfer memory transfer)
        private
        pure
        returns (uint256)
    {
        return
            uint256(
                (transfer.options >> OPTIONS_OFFSET_EXPIRATION) &
                    OPTIONS_MASK_EXPIRATION
            );
    }
}
